...
def parse_film_page(page_url):
    ...
    for rating_parent in soup.find_all("div", {"class": "review-card-review-holder"}):
        rating_raw = rating_parent.find("span", {"class": "stareval-note"})  
        rating_str = str(rating_raw.contents)[2:5]  
    ...
    for helpful_raw in soup.find_all("div", {"class": "reviews-users-comment-useful js-useful-reviews"}):
        helpful_str = helpful_raw.get("data-statistics")  
        helpful_dic = json.loads(helpful_str)  
        helpful = [helpful_dic["helpfulCount"], helpful_dic["unhelpfulCount"]]
